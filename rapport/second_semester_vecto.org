...
** Introduction
  ...
  Les Benchmarks traditionnels qui existent bien avant les NPB étaient généralement limités
  pour être spécialisés pour les ordinateurs vectoriels.Du coup malgré leurs capacités, ils ont toujours des
  ...
  Par conséquent, les NPB ont été développés afin de remédier à ce manque de performances ainsi que les
  insuffisances dans ces machines purement parallèles.
  
** Définition des NAS Benchmark parallèle

  Les NAS parallèle benchmarks (NPB) ont été développés au centre de la recherche de la NASA. Ce sont une suite de 
  benchmarks améliorés afin d'évaluer et d’améliorer les performances informatiques parallèles qui sont faible 
  dans les benchmarks existant avant ces NPB  souffrant d'insuffisances divers notamment en parallélismes et tailles mémoires.
  
** Evolution des NAS Benchmarks Parallèle

  Comme tous les outils et programmes, le NPB pendant son évolution et
  développement est passé par plusieurs phases et plusieurs versions améliorées
  avec le temps et dépendant des besoins et problèmes rencontrés ; pour cela
  dans ce qui suit on a détaillé les trois versions du NPB , tel que chaque
  spécification a des références plus améliorées par rapport à l’ancienne
  version.
...
*** La version NPB1:
  ...
  spécifiques pour l’ensemble des problèmes rencontrés à des points de références qui puissent assurer :
  ...
  - Facilité de travailler et de s’adapter avec les systèmes multicœurs fiables ,ainsi que la distribution
  et communication multicœurs.
  
*** La version NPB2:
  Des problèmes de performances ainsi que du parallélisme sont résolus,graçe à la version NPB1 ,  
  ...
  en apportant des implémentation de codes sources pour les benchmarks 
  implémentés dans NPB1.Donc la spécification NPB2 à complèté les spécifications du NPB1 et nous a permis :
  ...
  - Et enfin la version NPB2 a permis d’implémenter des codes basés sur MPI .
...
***  La version NPB3:
  ...
  Mais en plus de ça des implémentations qui étais en série dans NPB2 , sont améliorées et évoluées afin d’être en parallèle avec des 
  optimisations supplémentaire. Ainsi que NPB3 a rajouté de nouvelle références tel que : l’ajout d’outils
  de parallélisation multi-niveau et hybrides.et aussi un ensemble de benchmarks multi-zone,ainsi que l’allocation de mémoire
  dynamique.
...
** Résultats et discussions
  ...
  donc on est restés sur la classe "S". Donc comme vous le voyez ici on a utilisé comme compilateur la nouvelle version de verificarlo,
  c'est à dire la version vectorisée.
  On a pris comme exemple le benchmark "BT" et de classe "S". Pour pouvoir executer le benchmark, il nous a
  demandé un nombre de coeurs qui répresente le carré d'un nombre. La figure suivant représente le résumé 
  de la compilation du benchmark BT:
   ...
   Après avoir compilé et éxecuté le benchmarck BT, la vectorisation au niveau du programme est répresentée
   ...
   Le benchmark que nous avons testé est écrit avec le langage fotran et parallèlisé avec MPI. Du coup on peut constater
   sur cette figure que le niveau de véctorisation est null. Celle-ci peut être dû à la difference de la syntaxe du langage entre
   le langage C et fortran. 
   Pour bien observer le niveau de véctorisation sur le benchmark BT, on a récuperé une source de benchmarks qui est écrit avec
   le langace C et parallèlisé avec OpenMP.
   Les figures suivantes representent l'évaluation de la véctorisation au niveau du benchmark BT avec 8 threads:
   <<figure c>>
...
* Conclusion
  ...
  Le test que nous avons fait a montré que les opérations vont plus vite avec la véctorisation. À cause du manque de ressources 
  sur nos machines, on a été obligé de se limiter à des tests minimums. Mais ce qui serai intéressant est de pouvoir faire 
  des tests sur des supércalculateurs avec les différentes classe de benchmarks et de bien évaluer la performance de la machine ainsi
